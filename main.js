...
$(window).load(function() {
  ...
  let json = api.boa.run('commit-police.boa');
  $('#loading').hide();
  $('#content').show();
  ...
  for(let index in json.project_empty_logs) {
    count++;
    $('#numToShow').append(`<option value="${count}"> ${count} </option>`);
    chartData.push({
        label: index,
        value: json.project_empty_logs[index],
  ...
  $('#app-title').html(`Number of empty Commit logs for ${instance.repos.name}`);
